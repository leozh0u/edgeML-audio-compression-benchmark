...
**Why 6.1 s and not the ~300 ms I estimated:** the estimate was simply wrong — it assumed optimized kernels. `tanakamasayuki/TensorFlowLite_ESP32` ships only **reference** int8 kernels (no SIMD); its `kernels/esp_nn/` directory is a README placeholder, not the real esp-nn implementation. The tiny model is ~240 M int8 MACs of conv; reference kernels at ~single-MAC-per-few-cycles put that in the multi-second range, and the identical per-clip latency (6148.2 ms every time, input-independent) confirms it's pure compute, not the mel front end. **The fix is Espressif's `esp-tflite-micro`, which bundles esp-nn (S3 SIMD kernels) — implemented next in M10, where it lands a 43–54× speedup and beats the original estimate.**

**Deployment decision (M9 — REVISED in M10):** at this point, ship the **tiny** model on-device (fits fast internal SRAM; mid is memory-bound in PSRAM at 22.5 s). This is the accuracy/latency/**memory** trilemma with silicon numbers, not host proxies. **M10 overturns this: esp-nn makes mid run at 416 ms, so the deployed model becomes mid (70%).** The reasoning below is the pre-esp-nn snapshot.

**Resume numbers at M9 (SUPERSEDED by M10 — see that milestone for the final figures):**
- On-device model: **tiny INT8, 26.7 KB, 54%** (mid didn't fit fast RAM *before esp-nn*).
- On-device inference latency: **6.1 s** with reference kernels (M10: 143 ms tiny / 416 ms mid with esp-nn).
...
## Milestone 11: INMP441 I2S mic wired — the live-input step, and the last board risk
The one step host validation could never cover: real microphone capture (ADC noise, I2S clock/timing, gain staging). Wired an INMP441 MEMS mic over I2S and closed it. Two on-board fixes, both the kind you can only hit with hardware in hand:

1. **Pin bug from the pre-board scaffold.** `config.h` used `SD=GPIO32` — a standard I2S pin on the *classic* ESP32, copied from a generic wiring example. On the **ESP32-S3** GPIO26–37 are bonded to the in-package SPI flash and octal PSRAM (GPIO32 = a flash data line), so it's physically unusable. Moved the three I2S signals to free GPIOs **4 (SCK) / 5 (WS) / 6 (SD)**. Lesson: pin maps don't port across ESP32 variants — the S3's flash/PSRAM claim a whole GPIO block.

2. **Gain calibration, done from measured numbers not guesswork.** Added a peak/RMS diagnostic to the capture loop and read it off the board: at an I2S shift of `>>14`, a loud clap peaked at **31,661** — right against the int16 ceiling of 32,767, i.e. clipping. Dropped to `>>15` (halves amplitude): quiet room went 8,694 → 4,352 (confirming the halving), so the loud clap now maps to ~15,800 — clean headroom. **Why the exact gain barely matters:** the mel front end does `power_to_db(ref=max)` then per-clip z-norm, both of which are invariant to a constant scale on the PCM. So the shift only has to (a) not clip and (b) stay above the int16 quantization floor — both true at `>>15`. That's the principled version of "set the mic gain," not trial-and-error.

**Verified working end-to-end on real audio:** the mic drives the full on-device pipeline (I2S capture → C mel front end → esp-nn INT8 inference) at ~1.3 s/inference (888 ms mel + 416 ms inference), and responds sensibly to real sound — a clap classified as `fireworks`, a knock as `door_wood_knock` (correct *categories* of percussive/transient sound). Live predictions stream mic → USB serial → `server.py --serial` → WebSocket → the React dashboard, updating in the browser in real time. Note on accuracy: live-mic predictions are noisier than the 54–70% benchmark — expected domain shift (a ¥12 MEMS mic in an untreated room vs ESC-50's curated clips); the point of this milestone is that the *capture + on-device inference path* is correct, which the clap/knock category responses confirm.

**Project status: complete.** Everything from the compression sweep to on-device real-time inference from a live microphone is built, measured, and demonstrated on hardware. Remaining items are optional polish (WiFi bridge instead of USB serial; structured pruning; a larger mic-side denoise/AGC front end).
