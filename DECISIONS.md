...
## Repo hygiene: public README + private handoff + clean history
- Added `README.md` as the public front door: headline compression numbers, the PTQ-vs-QAT finding, the full 17-variant results table, a pipeline diagram, and repro steps. Rationale: a portfolio repo with no README reads as unfinished no matter how good the code is; the README is the one artifact a recruiter actually reads. Every number is sourced from `results/benchmark.json`, not restated from memory, so it can't drift from the harness output.
- `DECISIONS.md` is intentionally **not** in the repo (gitignored) and was scrubbed from git history. It's a working/onboarding doc that names the portfolio strategy; that belongs in local notes, not on a public profile. The public engineering narrative lives here in DECISIONS.md instead, which is why the few code references that pointed at DECISIONS.md were repointed here.
- Git history was rewritten (`git-filter-repo`) and force-pushed to remove development-tooling artifacts (an assistant-config file, co-author trailers, a working branch) so the history reads as a clean solo project. Verified: no orphaned references from PRs/forks, both private docs 404 on the remote. Lesson banked from the earlier `b5a155d` incident applies here too — history rewrites are surgical and irreversible on the remote, so a `.git` backup was taken before the rewrite.
