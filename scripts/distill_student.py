"""
Milestone 3: distill teacher_v2 into a tiny student for edge deployment.
"""
...
LR = 1e-3
TEMPERATURE = 4.0
ALPHA = 0.7  # weight on distillation loss vs hard-label loss
...
# ---- Teacher (must match milestone 2 architecture exactly) ----
...
# ---- Tiny student, target: a few hundred KB after quantization ----
...
def main():
    wandb.init(project="edge-ml-esc50", name="milestone3-distillation")
    ...
    student = TinyStudent(n_classes=50).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(student.state_dict(), os.path.join(CKPT_DIR, "student_distilled_best.pt"))

    print(f"best student val acc: {best_val_acc:.3f}")
    wandb.finish()
