...
def representative_dataset_gen(calib_files):
    for path in calib_files:
        ...
        x = mel[np.newaxis, np.newaxis, :, :]  # NCHW to match export shape
        yield [x]


def convert_to_tflite_int8(onnx_path, name, calib_files):
    import subprocess
    saved_model_dir = os.path.join(TFLITE_DIR, f"{name}_savedmodel")

    # onnx2tf converts ONNX -> TF SavedModel directly, CLI-based
    subprocess.run(
        ["onnx2tf", "-i", onnx_path, "-o", saved_model_dir, "-osd"],
        check=True,
    )
