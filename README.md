...
**Headline:** a 3.9 MB / 77.5% CNN teacher compressed to a **71 KB model at 70.8%**
(55× smaller) and a **25 KB model at 56.5%** (156× smaller).
...
1. **Post-training quantization made the smallest model *worse on both axes*.**
   TinyStudent (~17K params): 56.5% → 52.75% accuracy, and the file **grew**
   19.3 KB → 37.6 KB. ONNX Runtime's QDQ format inserts scale/zero-point nodes
...
2. **Quantization-aware training breaks through that floor.** Distillation-aware
   ...
   shrinking it 3.06× to 25 KB. On the Pareto plot, tiny-QAT sits on the
...
3. **Pruning tolerance tracks over-parameterization.** MidStudent (64K params)
   ...
   capacity floor — consistent evidence, not coincidence.
...
| TinyStudent INT8 PTQ | 16,962 | 52.75% | 37.6 KB ⬆ | *dominated* |
